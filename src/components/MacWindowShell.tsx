...
const trafficLightStyles: Record<TrafficLightVariant, { bg: string; border: string; ring: string }> = {
...
export default function MacWindowShell({
  ...
  const [shellState, setShellState] = useState<ShellState>(() => readPersistedShellState(storageKey));
  ...
  return (
    <>
      {isClosed && (
        <OpenPanelButton onClick={handleRestore} />
      )}
      ...
      {showPanel && (
        <div
          ...
          className={`fixed z-50 flex flex-col overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-gray-200 ${motionClass} ${
